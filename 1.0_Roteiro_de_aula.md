...
#### Arquivos utilizados nessa aula:
1. Planilha Favorites.csv e outros arquivos: https://cdn.cs50.net/2020/fall/lectures/7/src7.zip
1. BD Movies (IMDB) https://cdn.cs50.net/2021/x/seminars/sql/movies.db
1. BD Fiftyville (Desafio final) https://cdn.cs50.net/2021/x/seminars/sql/fiftyville.db


## Introdução
A primeira base de dados que vamos utilizar foi coleta por meio de um formulário (Google Forms). Neste formulário, foi solicitado que cada entrevistado respondesse o nome do programa de TV favorito e inserisse quais os gêneros desse programa.
...
Abertura do arquivos csv em aplicativo de texto (notepad) e planilha (Google Sheets)
...
Vamos iniciar executando o SGBD do sqlite para importar os dados para o BD. No terminal, digite:
```
sqlite3
```
...
No exercício anterior, não precisamos nos preocupar com o schema (estrutura) do BD, pois a importação foi realizada de forma automática. No entanto, é importante lembrar que um BD tem a função de otimizar as buscas por dados persistentes em disco.

Importante notar que muitos programas possuem seus nomes repetidos, assim como seus gêneros. Já vamos corrigir isso, mas antes...

Alguém consegue me dizer ONDE esse BD está salvo?

Um forma de prevenir isso, é criando-se primeiro a estrutura do BD depois popula-se com os dados. Em se tratando de BDs, é comum encontrarmos a expressão CRUD, que é o acrônimo de CREATE, READ, UPDATE, DELETE.
